...
def inspect(filepath, slide_num=None, include_text=False, include_notes=False, include_layouts=False):
    ...
    if slide_num is not None and not 1 <= slide_num <= len(prs.slides):
    ...
    for slide_number, slide in enumerate(prs.slides, start=1):
        if slide_num is not None and slide_number != slide_num:
            continue
        ...
        for shape in slide.shapes:
            ...
            try:
                image = shape.image
                s["has_image"] = True
                s["image_content_type"] = image.content_type
            except Exception:
                pass
...
def main():
    ...
    parser.add_argument("--slide", type=int, help="Inspect specific slide number (1-based, matching PowerPoint)")
