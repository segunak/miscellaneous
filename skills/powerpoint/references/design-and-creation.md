...
## Table of Contents
1. [Creation Engine Choice](#creation-engine-choice)
2. [Creating Slides with PptxGenJS](#creating-slides-with-pptxgenjs)
3. [Creating Slides with python-pptx](#creating-slides-with-python-pptx)
4. [Editing Existing Presentations](#editing-existing-presentations)
5. [Design Palettes](#design-palettes)
6. [Charts](#charts)
7. [Tables](#tables)
8. [Images & Media](#images--media)
9. [Layouts & Templates](#layouts--templates)
10. [Speaker Notes](#speaker-notes)
11. [Transitions & Animations](#transitions--animations)

## Creation Engine Choice

Use PptxGenJS for polished decks created from scratch. It gives better control over shape systems, image placement, icon rendering, chart styling, slide masters, and repeatable design tokens.

Use python-pptx for quick functional decks, simple content edits, reading, metadata inspection, and cases where the existing Python ecosystem is enough.

Use OOXML helpers for template surgery, slide order, low-level repair, and cases where preserving a source deck's native structure matters.

## Creating Slides with PptxGenJS

For polished from-scratch decks, start with `references/pptxgenjs-creation.md` and the bundled generator:
...
Before generating, commit to a specific design direction:

1. Audience and room setting.
2. One dominant color, one support color, one accent.
3. One repeated motif, such as icon bubbles, side bars, process blocks, or half-bleed images.
4. A slide mix that includes title, section, comparison, process, quote, chart or table, and close.
5. A QA plan for text extraction, structural validation, and visual review.

Do not treat PptxGenJS as a prettier bullet generator. Use it to make the deck feel designed.

## Creating Slides with python-pptx

## Design Planning

Before creating a deck, make a short design plan:

1. Define the audience and setting.
2. Pick one dominant visual direction and one accent color.
3. Map content to varied slide patterns before writing code.
4. Decide which slides need speaker notes.
5. Decide what must be inspected manually after generation.

Avoid a deck that is just title plus bullets repeated twenty times. Use structure: title, section divider, scenario prompt, comparison, quote, diagram, chart, process, table, and closing slides.

Every slide should have a clear visual role. That role can be a chart, table, icon row, process flow, large quote, split comparison, image, or intentionally sparse statement slide.
...
- Let one color dominate. Do not give every color equal weight.
- Use one accent color for emphasis, not decoration everywhere.
- Keep title and closing slides visually distinct from dense content slides.
- Avoid low-contrast text and icons.
- Avoid generic default blue unless it is intentionally part of the topic or brand.
...
| Pattern | Good For |
|---------|----------|
| Title | Opening, framing, memorable claims |
| Section divider | Major transitions |
| Two-column | Comparisons, before and after, tradeoffs |
| Scenario prompt | Audience interaction and classification |
| Quote | External authority or philosophical spine |
| Process flow | Steps, loops, agent workflows |
| Icon row | Related concepts with short labels |
| Chart | Quantitative evidence |
| Table | Structured comparison or rubric |
| Closing | Resources, QR codes, next steps |

Common mistakes:

- Reusing the same layout for every slide.
- Centering body text.
- Using tiny body text to fit too much content.
- Filling every inch of the slide.
- Leaving one edited slide polished while the rest of the deck stays plain.
- Adding decorative shapes that do not clarify the content.
